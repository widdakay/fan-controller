...
template<> inline String ConcreteSensorInstance<class Ina226, app::PowerReading>::formatReadingAsJson(const app::PowerReading& r) {
    if (!r.valid) return "{}";
    return "{\"bus_volts\":" + String(r.busVolts, 3) +
           ",\"shunt_mv\":" + String(r.shuntMillivolts, 3) +
           ",\"current_ma\":" + String(r.currentMilliamps, 1) +
           ",\"power_mw\":" + String(r.powerMilliwatts, 1) +
           ",\"load_volts\":" + String(r.loadVolts, 3) +
